    ...
    def save_task_workflow_schema_overrides(self, ft_project_schema, project_schema):
        ...
        for ft_task_workflow_schema_override in ft_project_schema['task_workflow_schema_overrides']:
            project_schema['task_workflow_schema_overrides'].append({
                ...
                'schema': self.get_add_workflow_schema(ft_task_workflow_schema_override['workflow_schema'])
            })
        ...
        for project_schema in self.result['project_schemas']:
            ...
            ft_project_schema = self.session.create('ProjectSchema', {
                ...
                'task_workflow_schema_id':
                    self.get_workflow_schema(project_schema['task_workflow_schema'])['id'],
                'task_type_schema_id':
                    self.get_task_schema(project_schema['task_type_schema'])['id'],
                'asset_version_workflow_schema_id':
                    self.get_workflow_schema(project_schema['asset_version_workflow_schema'])['id']
            })
            ...
            for object_type in project_schema['object_types']:
                ...
                if object_type['name'].lower() != 'milestone':
                    project_schema_object_type = {
                        ...
                        'object_type_id': self.get_object_type(object_type['name'])['id']
                    }
                    for key in ['icon', 'is_leaf', 'is_schedulable', 'is_statusable', 'is_taskable',
                                'is_time_reportable', 'is_typeable', 'sort']:
